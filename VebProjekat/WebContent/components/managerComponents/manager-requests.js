         ...
         <div class="tbl-content">
              <table class="r-table" cellpadding="0" cellspacing="0" border="0"  >
                   <tbody v-for="request in requests">
                        ...
                        <td v-if="request.status=='WAITING'"><button @click="approveRequest(request)" >Odobri</button><button @click="declineRequest(request)">Odbij</button> </td>
...
mounted : function() {
    axios
   .get('rest/requests/getAllRequestsForRestaurant')
...
methods:{
     approveRequest : function(request){
          ...
          axios
          .put('rest/requests/approveRequest/' + request.id)
          .then( response => {
               this.requests=[];
     ...
     declineRequest : function(request){
               ...
               axios
               .put('rest/requests/rejectRequest/' + request.id)
               .then( response => {
                    this.requests=[];
                    response.data.forEach(x => {
                         this.requests.push(x);
